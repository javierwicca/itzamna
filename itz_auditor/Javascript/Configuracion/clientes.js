...
function inicio() {
	parent.document.getElementById(window.name).parentNode.parentNode.parentNode.parentNode.title='ITZAMNNÁ AUDITOR - CLIENTES';
	cargarMenu('3','L');
	cargarPermisos ('3','cliente',['i_dojo_2','i_dojo_3','i_dojo_33','','i_dojo_0'],['1','1','1','1','1'],["document.getElementById('list_cliente').value"],
	"'&cliente='+base64_encode(document.getElementById('list_cliente').value)",['no'],['no'],['no'],['no'],'L');
}
...
function cargar_grid(datos) {
	...
	dojo.declare("dojox.grid.formatterScopeObj", null, {
		...
		fmtRadioButton: function(value, idx){
			...
			valores[1]='';
			valores=value.split('##');
			if (valores[0]==document.getElementById('list_cliente').value) {
				chk=true;
	...
	if (dijit.byId('grid_cliente')) {
		...
		if (datos!='') {
			...
			for (i=0;i<a_datos.length;i++) {
				usr=a_datos[i].split('@@');
				if (usr[0].length>len_cli) len_cli=usr[0].length;
			}
			...
			for (i=0;i<a_datos.length;i++) {
				...
				store.newItem({
					cd_cliente: str_replace("'", "\'", reempCaracEspDojo(usr[0]+'##'+usr[15]+'##'+cd)),
					identificacion: str_replace("'", "\'", reempCaracEspDojo(str_pad(usr[0],len_cli,'0','STR_PAD_LEFT')+'##'+usr[8])),
		...
		store=new dojo.data.ItemFileWriteStore({url: '../../Stores/consultasClientes.php',clearOnClose:true});
		...
		var objeto=new dojox.grid.formatterScopeObj({jsId:'objeto_cliente'});
		
		var columnas=[
									{ name: 'Sel.', field: 'cd_cliente', width: '4%',styles:"text-align: center;",formatter:"fmtRadioButton"},
	...
	document.getElementById('grid_cliente').style.height=alto+'px';
	dijit.byId('grid_cliente').render();
}
...
function mostrar_bot(valor) {
	...
	if (document.getElementById('td_mod')) document.getElementById('td_mod').style.display='';
	dijit.byId("b_mod_tooltip").attr('label','Modificar Cliente: '+reemp_carac_esp_html(valores[1]));
	
	if (document.getElementById('td_ina')) document.getElementById('td_ina').style.display='';
	dijit.byId("b_ina_tooltip").attr('label','Modificar Cliente: '+reemp_carac_esp_html(valores[1]));
	
	if (document.getElementById('td_inf')) document.getElementById('td_inf').style.display='';
	if (dijit.byId("b_inf_tooltip")) dijit.byId("b_inf_tooltip").attr('label','Informaci&oacute;n Cliente: '+reemp_carac_esp_html(strtoupper(valores[1])));
	
	document.f_consulta.list_cliente.value=valores[0];
}

function ver_cliente() {
	if (dijit.byId('i_dojo_2').value!='') {
		var peticion=false,fragment_url='../../Controlador/Control.php',envio='';
		peticion=object();
		peticion.open("POST", fragment_url, true);
		peticion.onreadystatechange = function(){ 
																							if (peticion.readyState == 4) {
																								if (peticion.responseText!='') {
																									...
																									mensaje_dj('ERROR','Cliente Digitado Ya Existe.','OK','ERROR','',dijit.byId('i_dojo_2'));
		...
		peticion.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
		envio='action='+base64_encode('consultarClientes')+'&list_t_cliente='+base64_encode(dijit.byId('i_dojo_2').value)+'&fechaInicio='+
...
function add_ciiu_ci() {
	...
	if (navigator.appName!='Microsoft Internet Explorer') {
		...
		td=document.createElement("<td class='info' colspan='5' id='td_v_ciiu_ci_"+filas.toString()+"'>");
		tr.appendChild(td);
	}
	document.getElementById("filas_dojo").value=filas_d;
}
...
function sel_rb(campo) {
	...
	while (document.getElementById('n_pri_ci_ci_'+i)) {
		if (document.getElementById('v_ciiu_ci_'+i).value=='S') {
			...
			campo.name==dijit.byId(document.getElementById('n_pri_ci_ci_'+i).value).name) dijit.byId(document.getElementById('n_pri_ci_ci_'+i).value).set('checked',false);
		}
		i++;
	}
}
...
function act_cliente() {
	if (!valida(dijit.byId('i_dojo_2'), 'Por favor digite el n&uacute;mero de identificaci&oacute;n del cliente.',false)) {
		dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
		return false;
	}
	
	if (dijit.byId('i_dojo_5').value=='J') {
		if (!valida(document.getElementById('i_dojo_3'), 'Por favor digite el digito de verificaci&oacute;n del cliente.',false)) {
			dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
			return false;
		}
	}
	...
	if (!valida(dijit.byId('i_dojo_4'), 'Por favor seleccione el tipo de documento del cliente.',false)) {
		dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
		return false;
	}
	
	if (!valida(dijit.byId('i_dojo_8'), 'Por favor seleccione el lugar de expedici&oacute;n del documento del cliente.',false)) {
		dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
		return false;
	}
	
	if (!valida(dijit.byId('i_dojo_9'), 'Por favor digite el nombre del cliente.',false)) {
		dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
		return false;
	}
	
	if (dijit.byId('i_dojo_5').value!='J') {
		if (!valida(dijit.byId('i_dojo_10'), 'Por favor digite los apellidos del cliente.',false)) {
			dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_1'));
			return false;
		}
	}
	...
	while (document.getElementById('n_dir_suc_'+i)) {
		if (document.getElementById('v_suc_'+i).value=='S') {
			...
			if (!valida(dijit.byId(document.getElementById('n_ciu_suc_'+i).value), 'Por favor digite la ciudad de la sucursal.',false)) {
				dijit.byId('i_dojo_0').selectChild(dijit.byId('i_dojo_37'));
				return false;
			}
		}
				...
				if (document.getElementById('v_ciiu_'+j).value=='S') {
					if (dijit.byId(document.getElementById('n_nom_ci_'+i).value).id!=dijit.byId(document.getElementById('n_nom_ci_'+j).value).id) {
						if (dijit.byId(document.getElementById('n_nom_ci_'+i).value).value==dijit.byId(document.getElementById('n_nom_ci_'+j).value).value) {
							...
							mensaje_dj('ERROR','Se encuentra repetida la actividad econ&oacute;mica.','OK','ERROR','',dijit.byId(document.getElementById('n_nom_ci_'+i)));
							return false;
						}
					}
				}
	...
	peticion.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	envio='ventana='+base64_encode('vCliente')+'&accion_d='+base64_encode(document.getElementById('accion_d').value)+'&accion_c='+
	base64_encode(document.getElementById('accion_c').value)+'&accion_r='+base64_encode(document.getElementById('accion_r').value)+'&datos_ant_d='+
	reemp_carac_esp_js(document.getElementById('datos_ant_d').value)+'&datos_ant_c='+
	reemp_carac_esp_js(document.getElementById('datos_ant_c').value)+'&datos_ant_r='+
	...
	base64_encode(reemp_carac_esp_js(document.getElementById('filas_ciiu').value))+'&filas_ciiu_ci='+
