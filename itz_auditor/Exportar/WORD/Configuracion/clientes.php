<?php
	...
	->setSubject(utf8_encode("Listado de Clientes."))
	->setDescription(utf8_encode("Listado de Clientes con la siguiente condición: $where."))
	->setKeywords(utf8_encode("CLIENTES"));
	...
	$table->addCell(2500,array('valign'=>'center'))->addText("CLIENTES",'fSize10','pCenter');
	...
	// Add cells
	$table->addCell(1500, $styleCell)->addText('Identificación', 'fBold','pCenter');
	$table->addCell(2500, $styleCell)->addText('Nombre', 'fBold','pCenter');
	$table->addCell(1200, $styleCell)->addText('Dirección', 'fBold','pCenter');
	$table->addCell(1000, $styleCell)->addText('Teléfono', 'fBold','pCenter');
	$table->addCell(1100, $styleCell)->addText('Persona', 'fBold','pCenter');
	$table->addCell(1100, $styleCell)->addText('Tp. Documento', 'fBold','pCenter');
	...
	for ($i=0;$i<count($filas);$i++) {
		...
		$table->addRow();
		$table->addCell(1500, $styleCell)->addText($columnas[0], $fontStyle,'pRight');
		$table->addCell(2500, $styleCell)->addText(mb_strtoupper($columnas[15]), $fontStyle,null);
		$table->addCell(1200, $styleCell)->addText(mb_strtoupper($columnas[9]), $fontStyle,null);
		$table->addCell(1000, $styleCell)->addText(mb_strtoupper($columnas[13]), $fontStyle,null);
		$table->addCell(1100, $styleCell)->addText(mb_strtoupper($columnas[16]), $fontStyle,null);
		$table->addCell(1100, $styleCell)->addText(mb_strtoupper($columnas[17]), $fontStyle,null);
	}
	...
	header('Content-Disposition: attachment;filename="Clientes.docx"');
